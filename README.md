...
| [`@design-parity/policy`](./packages/policy) | ✅ issue #12 | Committed `.design-parity.json` (schema, loader, validator CLI) + the deterministic `auto` → `design-led`/`code-led` direction resolver. Also carries the `tokens` comparison policy — missing-number handling, text-derived insets, and exact issue-backed accepted differences; see [token matching §4.3](./docs/correspondence-and-token-matching.md#43-how-matching-works-today-packagesdiffsrctokensts). |
