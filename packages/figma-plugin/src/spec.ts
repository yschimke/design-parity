...
export function specToIssueBody(spec: FrameSpec): string {
  ...
  lines.push("");
  lines.push("> **Attach the exported frame PNG** (the Handoff to code task offers it as a download) so the visual target is in this issue.");
  lines.push("");
  ...
  lines.push("");
  lines.push("---");
  lines.push("_Generated by the design-parity Figma plugin — Handoff to code._");
  return lines.join("\n");
}
